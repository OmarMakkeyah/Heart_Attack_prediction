...
def classify_image(model, image_path):
    ...
    with torch.no_grad():
        output = model(image)
        probabilities = torch.softmax(output, dim=1)  # Get softmax probabilities
        predicted_class = torch.argmax(probabilities).item()  # Get the predicted class
    ...
    return classes[predicted_class], probabilities[0]  # Return the predicted class and probabilities
...
def open_file_and_classify():
    ...
    if file_path:
        try:
            ...
            predicted_class, probabilities = classify_image(model, file_path)
            prediction_label.configure(text=f"Prediction: {predicted_class}")  # Update prediction label
            
            # Update softmax probabilities label
            probabilities_label.configure(text="Softmax Probabilities: " + 
                                           ", ".join(f"{p:.2f}" for p in probabilities.cpu().numpy()))  # Show probabilities
...
probabilities_label = ctk.CTkLabel(app, text="Softmax Probabilities: ", font=("Arial", 16))
